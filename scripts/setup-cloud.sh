...
# Generate secrets_local.py (single-node only; remote HA: user configures manually)
SSH_KEY_PATH="$PROJ_DIR/deploy/config/ssh/id_rsa"
chmod 600 "$SSH_KEY_PATH" 2>/dev/null || true

if [ "$CLUSTER_MODE" = "1" ]; then
  cat > "$PROJ_DIR/src/secrets_local.py" << EOF
"""Auto-generated by setup-cloud.sh"""
import os
...
EOF
  echo "  secrets_local.py generated"
else
  echo "  [Remote HA] 请手动编辑 src/secrets_local.py"
fi
